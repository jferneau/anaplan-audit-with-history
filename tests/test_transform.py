...
from anaplan_audit.transform.loader import _sanitize_for_sqlite, load_to_sqlite
...
def _seed_full_schema(db_path: Path) -> None:
    ...
    Uses ``pd.DataFrame.to_sql`` directly so that pandas handles column
    quoting (including dotted ``additionalAttributes.*`` names) without
    going through the custom upsert path.  The loader is tested separately
    in ``TestSQLiteLoader``.
    """
    ...
    with closing(sqlite3.connect(str(db_path))) as conn:
        ...
        workspaces_df.to_sql("workspaces", conn, if_exists="replace", index=False)
        models_df.to_sql("models", conn, if_exists="replace", index=False)
        cloudworks_df.to_sql("cloudworks", conn, if_exists="replace", index=False)
        act_codes_df.to_sql("act_codes", conn, if_exists="replace", index=False)
...
class TestSQLiteLoader:
    ...
    def test_events_upsert_preserves_history(self, tmp_path: Path) -> None:
        ...
        load_to_sqlite(db_path, {"events": batch2})
    ...
    def test_known_optional_columns_predeclared(self, tmp_path: Path) -> None:
        ...
        load_to_sqlite(db_path, {"events": batch})
    ...
    def test_wal_mode_enabled(self, tmp_path: Path) -> None:
        """WAL journal mode is applied for performance."""
        db_path = tmp_path / "test.db"
        load_to_sqlite(db_path, {"workspaces": pd.DataFrame([{"id": "ws-1", "name": "Test"}])})
        with closing(sqlite3.connect(str(db_path))) as conn:
    ...
    def test_nested_dict_column_serialised_to_json(self, tmp_path: Path) -> None:
        ...
        load_to_sqlite(db_path, {"users": users_df})
    ...
    def test_nested_list_column_serialised_to_json(self, tmp_path: Path) -> None:
        ...
        load_to_sqlite(db_path, {"models": df})
    ...
    def test_error_message_includes_table_name(
        ...
        """SQLiteLoadError context includes the failing table name."""
        db_path = tmp_path / "test.db"

        # Make to_sql raise *after* current_table is set in the loop so the
        # table name makes it into the error context.
        def _explode(self: pd.DataFrame, *args: object, **kwargs: object) -> None:
            raise RuntimeError("simulated write failure")

        monkeypatch.setattr(pd.DataFrame, "to_sql", _explode)
        bad_df = pd.DataFrame([{"id": "1"}])
        with pytest.raises(SQLiteLoadError) as exc_info:
            load_to_sqlite(db_path, {"broken_table": bad_df})
        assert "broken_table" in exc_info.value.context.get("table", "")


class TestSanitizeForSQLite:
    """Unit tests for the _sanitize_for_sqlite helper."""

    def test_scalar_columns_unchanged(self) -> None:
        df = pd.DataFrame([{"id": "1", "name": "Alice", "active": True}])
        result = _sanitize_for_sqlite(df)
