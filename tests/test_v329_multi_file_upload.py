...
def _write_metadata_db(db_path: Path) -> None:
    with closing(sqlite3.connect(str(db_path))) as conn:
        pd.DataFrame([{"id": "w1", "name": "WS"}]).to_sql("workspaces", conn, index=False)
        pd.DataFrame([{"id": "u1", "userName": "a@b.co"}]).to_sql("users", conn, index=False)
        pd.DataFrame([{"id": "m1", "name": "Model"}]).to_sql("models", conn, index=False)
        pd.DataFrame([{"id": "a1", "name": "Import A", "type": "import", "model_id": "m1"}]).to_sql(
            "actions", conn, index=False
        )
        pd.DataFrame([{"integrationId": "cw1", "name": "Sync"}]).to_sql(
            "cloudworks", conn, index=False
        )
        pd.DataFrame([{"Event Code": "USR-8", "Event Message": "Login"}]).to_sql(
            "act_codes", conn, index=False
        )
