...
from anaplan_audit.transform.loader import load_to_sqlite
...
class TestUserNarrow:
    def test_declared_fields_are_the_intentional_three(self) -> None:
        # Quinn's narrow. Any expansion needs a documented reason per
        # spec Section 3.2 / 8.
        assert set(User.model_fields.keys()) == {"id", "userName", "displayName"}

    def test_scim_extras_do_not_reach_dumped_dict(self) -> None:
        ...
        assert set(user.model_dump().keys()) == {"id", "userName", "displayName"}
...
def _seed_models_users_workspaces(db_path: Path) -> None:
    ...
    load_to_sqlite path so the view creation runs.
    ...
    users_df = pd.DataFrame(
        [
            {"id": "usr-alice", "userName": "alice@example.com", "displayName": "Alice"},
            {"id": "usr-bob", "userName": "bob@example.com", "displayName": "Bob"},
        ]
    )
...
class TestPragmaTableInfoAfterLoad:
    ...
    def test_users_table_is_narrowed_to_three_columns(self, tmp_path: Path) -> None:
        db_path = tmp_path / "test.db"
        _seed_models_users_workspaces(db_path)
        cols = _table_columns(db_path, "users")
        assert cols == {"id", "userName", "displayName"}
...
class TestModelsExportView:
    def test_view_is_created_on_load(self, tmp_path: Path) -> None:
        ...
        with closing(sqlite3.connect(str(db_path))) as conn:
            row = conn.execute(
                "SELECT name FROM sqlite_master WHERE type='view' AND name='v_models_export'"
            ).fetchone()
    ...
    def test_view_resolves_email_and_display_name_via_left_join(self, tmp_path: Path) -> None:
        ...
        with closing(sqlite3.connect(str(db_path))) as conn:
            ...
            row = conn.execute("SELECT * FROM v_models_export WHERE id = 'mod-known'").fetchone()
        assert row["lastModifiedByEmail"] == "alice@example.com"
        assert row["lastModifiedByDisplayName"] == "Alice"
    ...
    def test_left_join_preserves_row_with_unknown_guid(self, tmp_path: Path) -> None:
        ...
        with closing(sqlite3.connect(str(db_path))) as conn:
            conn.row_factory = sqlite3.Row
            row = conn.execute("SELECT * FROM v_models_export WHERE id = 'mod-orphan'").fetchone()
        # The row is present — spec acceptance criterion #3.
        assert row is not None
        assert row["id"] == "mod-orphan"
        # Email columns are null (SQLite None) because the GUID didn't
        # match any user.
        assert row["lastModifiedByEmail"] is None
        assert row["lastModifiedByDisplayName"] is None
